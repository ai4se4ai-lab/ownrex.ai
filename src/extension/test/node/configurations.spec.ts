...
describe('Configurations', () => {
	...
	it('settings in code should match package.json', () => {
		...
		internalKeys.forEach(key => {
			expect(key, 'Internal settings must start with ownrex.ai.chat.advanced.').toMatch(/^github\.copilot\.chat\.advanced\./);
		});
		...
		advancedPublicKeys.forEach(key => {
			expect(key, 'Advanced settings must not start wih ownrex.ai.chat.advanced.').not.toMatch(/^github\.copilot\.chat\.advanced\./);
