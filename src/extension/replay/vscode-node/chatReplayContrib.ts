...
export class ChatReplayContribution extends Disposable {
	...
	constructor(
		...
		const chatParticipant = chat.createChatParticipant('ownrex.ai.chatReplay', replayParticipant.handleRequest.bind(replayParticipant));
		...
		commands.executeCommand('setContext', 'ownrex.ai.chat.replay.workspaceEditTracing', false);

		this.registerDisplayChatFromLogCommand();
	}

	private registerDisplayChatFromLogCommand() {
		this._register(commands.registerCommand('ownrex.ai.chat.showAsChatSession', async (logFilePath: Uri) => {
	...
	private registerStartReplayCommand() {
		this._register(commands.registerCommand('ownrex.ai.chat.replay', async () => {
	...
	private registerEnableWorkspaceEditTracingCommand() {
		this._register(commands.registerCommand('ownrex.ai.chat.replay.enableWorkspaceEditTracing', async () => {
			...
			await commands.executeCommand('setContext', 'ownrex.ai.chat.replay.workspaceEditTracing', true);
		}));
	}

	private registerDisableWorkspaceEditTracingCommand() {
		this._register(commands.registerCommand('ownrex.ai.chat.replay.disableWorkspaceEditTracing', async () => {
			const logger = this._instantiationService.invokeFunction(accessor => accessor.get(IRequestLogger));
			logger.disableWorkspaceEditTracing();
			await commands.executeCommand('setContext', 'ownrex.ai.chat.replay.workspaceEditTracing', false);
		}));
	}
}
