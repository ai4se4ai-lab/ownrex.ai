...
export class VSCodeConfigProvider extends ConfigProvider {
	...
	override onDidChangeCopilotSettings: ConfigProvider['onDidChangeCopilotSettings'] = transformEvent(
		...
		event => {
			if (event.affectsConfiguration('ownrex.ai')) {
				return this;
			}
			if (event.affectsConfiguration('ownrex.ai-chat')) {
				return this;
			}
		}
	);
...
export async function enableCompletions(accessor: ServicesAccessor) {
	...
	// The rest of this function is the inverse of disableCompletions(), updating the ownrex.ai.enable setting.
...
/**
 * Disable completions using the ownrex.ai.enable setting.
 */
