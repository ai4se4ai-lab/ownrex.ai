...
export class DebugCommandsContribution extends Disposable {
	constructor(
		...
		this._register(vscode.commands.registerCommand('ownrex.ai.debug.generateSTest', async () => {
		...
		this._register(vscode.commands.registerCommand('ownrex.ai.createLaunchJsonFileWithContents', async (launchConfig: IStartDebuggingParsedResponse) => {
		...
		this._register((vscode.commands.registerCommand('ownrex.ai.startDebugging', async (config: IStartDebuggingParsedResponse, progress) => {
