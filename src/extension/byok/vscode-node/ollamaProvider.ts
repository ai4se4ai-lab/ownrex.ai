...
export class OllamaLMProvider extends BaseOpenAICompatibleLMProvider {
	...
	protected override async getAllModels(): Promise<BYOKKnownModels> {
		try {
			...
			throw new Error('Failed to fetch models from Ollama. Please ensure Ollama is running. If ollama is on another host, please configure the `"ownrex.ai.chat.byok.ollamaEndpoint"` setting.');
		}
	}
