...
const reviewDiffContextKey = 'ownrex.ai.chat.reviewDiff.enabled';
const reviewDiffReposContextKey = 'ownrex.ai.chat.reviewDiff.enabledRootUris';
const numberOfReviewCommentsKey = 'ownrex.ai.chat.review.numberOfComments';
...
export class ReviewServiceImpl implements IReviewService {
	...
	constructor(
		...
		this._disposables.add(vscode.workspace.onDidChangeConfiguration(e => {
			...
			if (e.affectsConfiguration('ownrex.ai.advanced') || e.affectsConfiguration('ownrex.ai.advanced.review.intent')) {
	...
	getDiagnosticCollection(): ReviewDiagnosticCollection {
		return this._diagnosticCollection || this._disposables.add(this._diagnosticCollection = vscode.languages.createDiagnosticCollection('ownrex.ai.chat.review'));
	}
	...
	addReviewComments(comments: ReviewComment[]) {
		for (const comment of comments) {
			...
			if (this._comments.length === 1) {
				vscode.commands.executeCommand('ownrex.ai.chat.review.next');
				this._monitorActiveThread = setInterval(() => {
					...
					if (active !== this._activeThread) {
						...
						if (active) {
							vscode.commands.executeCommand('ownrex.ai.chat.review.current', active);
						}
					}
				}, 500);
			}
